    ...
    extends Message
{
    ...
    public final static MessageDetails<CameraInfo> DETAILS = new MessageDetails<CameraInfo>(Name.parseMessageName("sensor_msgs/CameraInfo"), "c9a58c1b0b154e0e6da7578cb991d214", "CameraInfo message", (CameraInfo.class));
    /**
     *  Header timestamp should be acquisition time of image# Header frame_id should be optical frame of camera# origin of frame should be optical center of camera# +x should point to the right in the image# +y should point down in the image# +z should point into the plane of the image########################################################################                      Calibration Parameters                         ######################################################################### These are fixed during camera calibration. Their values will be the ## same in all messages until the camera is recalibrated. Note that    ## self-calibrating systems may "recalibrate" frequently.              ##                                                                     ## The internal parameters can be used to warp a raw (distorted) image ## to:                                                                 ##   1. An undistorted image (requires D and K)                        ##   2. A rectified image (requires D, K, R)                           ## The projection matrix P projects 3D points into the rectified image.######################################################################### The image dimensions with which the camera was calibrated. Normally# this will be the full camera resolution in pixels.
     * 
     */
    ...
    /**
     *  The distortion model used. Supported models are listed in# sensor_msgs/distortion_models.h. For most cameras, "plumb_bob" - a# simple model of radial and tangential distortion - is sufficent.
     * 
     */
     ...
     * 
     */
    public double[] P = new double[] {};
    /**
     * 
     * 
     */
    public long binning_x = 0L;
    /**
    ...
    public long getLength() {
        ...
        length += 4;
        length += 4;
        length += (distortion_model.length()+ 4);
        length += 4;
        ...
        throws IOException, RosMessageSerializationException
    {
        ...
        dataWriter.writeUInt32(distortion_model.length());
        dataWriter.writeString(distortion_model);
        dataWriter.writeUInt32(D.length);
        for (int index = 0; (index<D.length); index ++) {
            dataWriter.writeFloat64(D[index]);
        }
        dataWriter.writeUInt32(K.length);
        for (int index = 0; (index<K.length); index ++) {
            dataWriter.writeFloat64(K[index]);
        }
        dataWriter.writeUInt32(R.length);
        for (int index = 0; (index<R.length); index ++) {
            dataWriter.writeFloat64(R[index]);
        }
        dataWriter.writeUInt32(P.length);
        for (int index = 0; (index<P.length); index ++) {
            dataWriter.writeFloat64(P[index]);
        }
        dataWriter.writeUInt32(binning_x);
        dataWriter.writeUInt32(binning_y);
        roi.serialize(dataWriter);
    }
